		...
		for indexContent := 2; indexContent < len(block.B_content); indexContent++ {
			...
			if len(parentsDir) != 0 {
				...
				if strings.EqualFold(contentName, parentDirName) {
					...
					err := sb.createFolderInInodeExt3(path, content.B_inodo, utilidades.RemoveElement(parentsDir, 0), destDir)
					if err != nil {
						return err
					}
					return nil
		...
		for indexContent := 2; indexContent < len(block.B_content); indexContent++ {
			...
			if len(parentsDir) != 0 {
				...
				if strings.EqualFold(contentName, parentDirName) {
					...
					// Si son las mismas, entonces entramos al inodo que apunta el bloque
					err := sb.createFileInInodeExt3(path, content.B_inodo, utilidades.RemoveElement(parentsDir, 0), destFile, fileSize, fileContent)
					if err != nil {
						return err
					}
					return nil
