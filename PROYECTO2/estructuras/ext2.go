...
func (sb *SuperBlock) CreateUsersFileExt2(path string) error {
	...
	// ----------- Creamos /users.txt -----------
	usersText := "1,G,root\n1,U,root,root,123\n" //ACORDARSE DE CORREGIR ESTO SIEMPRE
	...
	for _, blockIndex := range inode.I_block {
		...
		for indexContent := 2; indexContent < len(block.B_content); indexContent++ {
			...
			if len(parentsDir) != 0 {
				...
				parentDir, err := utilidades.First(parentsDir)
				if err != nil {
					return err
				}
				...
				if strings.EqualFold(contentName, parentDirName) {
					...
					err := sb.createFolderInInodeExt2(path, content.B_inodo, utilidades.RemoveElement(parentsDir, 0), destDir)
					if err != nil {
						return err
					}
					return nil
		...
		for indexContent := 2; indexContent < len(block.B_content); indexContent++ {
			...
			if len(parentsDir) != 0 {
				...
				if strings.EqualFold(contentName, parentDirName) {
					...
					// Si son las mismas, entonces entramos al inodo que apunta el bloque
					err := sb.createFileInInodeExt2(path, content.B_inodo, utilidades.RemoveElement(parentsDir, 0), destFile, fileSize, fileContent)
					if err != nil {
						return err
					}
					return nil
