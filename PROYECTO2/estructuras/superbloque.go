...
func (sb *SuperBlock) VerifyUsersFile(usersInode *Inode, partitionPath string) (string, error) {
	// Leer el contenido de users.txt
	fileBlock := &FileBlock{}
	err := fileBlock.Deserialize(partitionPath, int64(sb.S_block_start+usersInode.I_block[0]*sb.S_block_size))
	if err != nil {
	...
	// Preparar el mensaje de verificación
	verificationMsg := "Contenido actualizado de users.txt:\n"
	verificationMsg += "-----------------------------\n"
	verificationMsg += content + "\n"
	verificationMsg += "-----------------------------\n"
...
func (sb *SuperBlock) CreateFile(path string, parentsDir []string, destFile string, size int, cont []string) error {
	...
	if sb.S_filesystem_type == 3 {
		...
		// Si parentsDir está vacío, solo trabajar con el primer inodo que sería el raíz "/"
		if len(parentsDir) == 0 {
			return sb.createFileInInodeExt2(path, 0, parentsDir, destFile, size, cont)
		}

		// Iterar sobre cada inodo ya que se necesita buscar el inodo padre
		for i := int32(0); i < sb.S_inodes_count; i++ {
			err := sb.createFileInInodeExt2(path, i, parentsDir, destFile, size, cont)
			if err != nil {
				return err
			}
		}
